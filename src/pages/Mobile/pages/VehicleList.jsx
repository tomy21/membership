...
export default function VehicleList() {
    ...
    const handleModalRfid = async (platnumber) => {
    ...
    const handleUpdateManual = async () => {
        try {
            setIsLoading(true);
            const response = await vehicleAdd.udpatedRFID(
                platNumber,
                manualRfid
            );
        ...
        try {
            ...
            if ('NDEFReader' in window) {
                ...
                ndef.addEventListener('reading', (event) => {
                    ...
                    const formattedRfid = serialNumber
                        .replace(/:/g, '')
                        .toUpperCase();
    ...
    const updateRfid = async (rfid) => {
        try {
            ...
            if (selectedVehicleId) {
                const response = await vehicleAdd.udpatedRFID(platNumber, rfid);
                if (response.status === true) {
                    setIsLoading(false);
                    fetchData();
                    setIsModal(true);
    ...
    return (
        <>
            <div className="flex flex-col justify-start items-center w-full">
                ...
                <div className="flex flex-col space-y-2 items-start justify-start w-full px-2 py-2 max-h-full overflow-y-auto">
                        ...
                        listVehicle.map((items, index) => (
                            ...
                            >
                                ...
                                <div className="flex justify-between items-center w-full mb-3 border-b border-dashed py-3 px-3">
                                    <h1>
                                        {items.member_customer_no.toUpperCase()}
                                    </h1>
                                    <h3>
                                        {format(
                                            new Date(items?.updatedAt),
                                            'dd MMM yyyy HH:mm'
                                        )}
                                    </h3>
                                </div>
