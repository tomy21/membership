...
export default function TableCard() {
  ...
  const autoConnectToSerialDevice = async () => {
    try {
      ...
      console.error("Failed to auto-connect to the serial device:", error);
    }
  };
  ...
  const handleConnectDevice = async () => {
    try {
      const port = await navigator.serial.requestPort();
      localStorage.setItem("serialDevice", JSON.stringify(port.getInfo()));
      await connectToPort(port);
    } catch (error) {
                      ...
                      <div className="flex flex-row justify-start items-center gap-x-3">
                        <div
                          className={`relative h-4 w-4 rounded-full text-xs flex items-center justify-center ${
                            item.is_used ? "bg-green-100" : "bg-yellow-100"
                          }`}
                        >
                          <div
                            className={`h-2 w-2 rounded-full ${
                              item.is_used ? "bg-green-500" : "bg-yellow-500"
                            }`}
                          ></div>
                        </div>
                        <p
                          className={`${
                            item.is_used ? "text-green-500" : "text-yellow-500"
                          }`}
                        >
            ...
            <div className="border w-full h-60 mt-3 bg-slate-200 rounded-lg flex items-center justify-center">
              {deviceData ? (
                isLoading ? (
                  ...
                  <div className="flex flex-col justify-center items-center p-3 space-y-3">
                    <RiRfidLine size={60} className="text-blue-600" />
                    <p className="text-center text-gray-700">{deviceData}</p>
                  </div>
                )
              ) : (
                <div className="flex flex-col justify-center items-center p-3 space-y-3">
